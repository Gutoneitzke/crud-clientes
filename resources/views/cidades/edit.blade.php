    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('cidades.update', ['id'=>$cidade->id]) }}">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="nome">Nome:</label>
                        <input type="text" name="nome" id="nome" value="{{ $cidade->nome }}" class="form-control" placeholder="Informe um nome para a cidade" required>
                    </div>
                    <br>
                    <div class="form-group">
                        ...
                        <select class="form-control" id="estado" name="estado_id">
                            @foreach($estados as $estado)
                                <option value="{{ $estado->id }}" {{ $cidade->estado_id == $estado->id ? 'selected' : '' }}>{{ $estado->nome }}</option>
                            @endforeach
                        </select>
                    </div>
                    <br>
