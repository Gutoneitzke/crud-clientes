    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('estados.update', ['id'=>$estado->id]) }}">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="nome">Nome:</label>
                        <input type="text" name="nome" id="nome" value="{{ $estado->nome }}" class="form-control" placeholder="Informe um nome para o estado" required>
                    </div>
                    <br>
                    <div class="form-group">
                        ...
                        <select class="form-control" id="pais" name="pais_id">
                            @foreach($paises as $pais)
                                <option value="{{ $pais->id }}" {{ $estado->pais_id == $pais->id ? 'selected' : '' }}>{{ $pais->nome }}</option>
                            @endforeach
                        </select>
                    </div>
                    <br>
